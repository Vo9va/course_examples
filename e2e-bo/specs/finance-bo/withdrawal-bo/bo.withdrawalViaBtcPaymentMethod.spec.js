...
describe('C25576 Financial Flow - Withdrawal via BTC payment method', function () {
  let cid;
  const { secureCode } = config.default;
  let btcPaymentMethodId = 5;
  ...
  it('Approve withdrawal via BTC method with free', async function () {
    const withdrawalApproveRes = await boWalletHelper.approveWithdrawalViaAnyMethodWithFee(
      agent,
      cid,
      withdrawalRequestId,
      walletDataBO.withdrawalViaBTCMethodWithFee
    );
  ...
  it('Wait for get transactions length change after approved withdrawal', async function () {
    ...
    expect(withdrawalTransactionAmount).to.equal(
      withdrawalAmount - walletDataBO.withdrawalViaBTCMethodWithFee.fee_amount
    );
    expect(withdrawalPaymentMethodId).to.equal(btcPaymentMethodId);
    expect(withdrawalTransactionFeeAmount).to.equal(walletDataBO.withdrawalViaBTCMethodWithFee.fee_amount);
  });
  ...
  it('Check transaction via filter in Operations -> Transactions', async function () {
    let resOperationTransactions = await boWalletHelper.getOperationTransactions(
      agent,
      `&id=${withdrawalTransactionId}&payment_method_id=${btcPaymentMethodId}`
    );
