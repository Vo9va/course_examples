...
describe('C19575 KYC Flow ( Add/Edit Documents ) - approve Documents', function () {
  let cid;
  ...
  it('Declined customer documents via BO', async function () {
    await boCustomerHelper.declineCustomerDocuments(agent);
  });
  ...
  it('Check customer documents', async function () {
    const resApprovedDocuments = await boCustomerHelper.getCustomerDocuments(
      agent,
      cid,
      constants.DOCUMENT_STATUS.DECLINED
    );
