...
//TODO not available on prod for Capitalix/InvestFW
describe('C30569 Proline: Create a new active customer', function () {
  let cid;
  const { prolineToken } = config.default;
  ...
  it('Create customer with Proline token and check response structure', async function () {
    const res = await customersHelper.createCustomerWithToken(agent, prolineToken, customer, {
  ...
  it('Check "affiliates.customers" table data', async function () {
    const resAffiliates = await affiliatesHelper.waitForAffiliatesCustomerData(agent, prolineToken, cid);
  ...
  it('Check "back_office.customers" table data', async function () {
    ...
    expect(resCustomerBo.body.customer.registration_source).to.equal('proline');
  });
});
