...
describe('C22365 Trading in Real Mode - edit Position (Stop Loss/Take Profit)', function () {
  let positionId;
  let realAccountId;
  let profit;
  let invested;
  ...
  it('Edit Position (Stop Loss/Take Profit)', async function () {
    const resUpdate = await tradingHelper.updatePosition(agent, positionId, walletData.updatePositionDataBuy);
    expect(resUpdate.statusCode).to.equal(200);

    let positions = await tradingHelper.waitForPositionPriceTpChange(agent, walletData.updatePositionDataBuy.price_tp);
    expect(positions.statusCode).to.equal(200);
    expect(positions.body.positions[0]).to.include({
      price_sl: walletData.updatePositionDataBuy.price_sl,
      price_tp: walletData.updatePositionDataBuy.price_tp,
    });
  });
