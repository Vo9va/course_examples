...
describe('C18612 Trading in Virtual Mode - edit Position (Stop Loss"/"Take Profit)', function () {
  let positionId;
  let accountId;
  let profit;
  let invested;
  ...
  let customerProd = customerData.getCustomerForLoginOnProd(5, brand);
  ...
  it('Edit Position (Stop Loss"/"Take Profit)', async function () {
    let resUpdate = await tradingHelper.updatePosition(agent, positionId, walletData.updatePositionDataSell);

    expect(resUpdate.statusCode).to.equal(200);

    let positions = await tradingHelper.waitForPositionPriceTpChange(agent, walletData.updatePositionDataSell.price_tp);

    expect(positions.statusCode).to.equal(200);
    expect(positions.body.positions[0]).to.include({
      price_sl: walletData.updatePositionDataSell.price_sl,
      price_tp: walletData.updatePositionDataSell.price_tp,
    });
  });
